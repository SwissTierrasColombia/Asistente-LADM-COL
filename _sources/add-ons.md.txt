...
Se procederá a instalar tanto el Asistente LADM-COL, cómo el Add-on siendo primero el Asistente LADM-COL (importante resaltar que debe instalarse la versión experimental) y luego el Add-on de Ambiente.

Al instalar el asistente, solicitará unas dependencias, se dará aceptar a las opciones por defecto, al terminar la primera instalación saldrá el siguiente menú.

<a class="" data-lightbox="Seleccionar rol" href="_static/add_ons/screenshot_03.png" title="Seleccionar rol" data-title="Seleccionar rol"><img src="_static/add_ons/screenshot_03.png" class="align-center" width="800px" alt="Seleccionar rol"/>

Aquí se seleccionará la opción Básico temporalmente y se dará aceptar.

Luego se instala el Add-on de Ambiente, este no requiere ningún paso adicional, una vez instalado se podrá cerrar la ventana de complementos, se podrá observar el siguiente Menú dentro de QGIS:
...
El siguiente ejercicio será cargar las capas que servirán de insumo para cargar a la base de datos:
...
Se procede a configurar la base de datos a utilizar, la cual puede ser GeoPackage o PostgreSQL.
...
La herramienta descargará automáticamente una herramienta llamada ili2gpkg y creará el esquema de base de datos, al finalizar se mostrará el siguiente mensaje:
...
Lo siguiente será dar clic en el botón Cerrar. Si se vuelve a dar clic en el menú de LADM-COL Ambiente, aparecerán nuevas funcionalidades:
...
Lo siguiente será dar clic en la opción en "Ejecutar ETL Ley 2da", esto cargará de forma automática las capas de salida a la ventana de la ETL y sólo será necesario mapear las capas de entrada que corresponden a la información insumo:
...
Lo siguiente será dar clic en "Ejecutar" (este proceso tarda alrededor de 5 minutos o menos), el Asistente LADM-COL correrá una serie de geoprocesos para transformar la información de las capas hacia el modelo de Reservas de Ley 2da, el resultado será el siguiente:
...
Una vez cargada la información, será posible revisar la información cargada dentro de cada una de las capas, e inclusive será posible hacer consultas a través de SQL.
...
Es posible que salga una ventana como esta, a la cual se le puede dar ignorar:
...
Una vez señalado el archivo de destino se dará clic en el botón Exportar datos y empezará el proceso de la generación del XTF, este proceso puede ser un poco largo (alrededor de 45 minutos).
...
Al terminar el proceso, el resultado será un "Export done" y se tendrá un archivo XTF válido para el modelo de reservas de Ley 2da (este archivo es un poco pesado debido a la cantidad de vertices que contienen las geometrías, así que si se desea abrir el texto plano, puede que tarde también un poco). El archivo XTF es un archivo de transferencia de datos, que se puede comprimir para compartir con otros actores, quienes podran importarlo a su propia base de datos.
