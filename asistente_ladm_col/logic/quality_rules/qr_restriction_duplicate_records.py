...
class QRDuplicateRestrictionRecords(AbstractLogicQualityRule):
    ...
    def __init__(self):
        ...
        self._name = "Revisar que la tabla restricción no tenga registros repetidos"
    ...
    def _validate(self, db, db_qr, layer_dict, tolerance, **kwargs):
        ...
        fields = LayerConfig.get_logic_consistency_tables(db.names).get(db.names.LC_RESTRICTION_T)
        # Check restriction with duplicate records
        res, records = ladm_queries.get_duplicate_records_in_table(db,db.names.LC_RESTRICTION_T,fields)
        count = len(records)
        ...
        if res:
            ...
            for record in records:
                error_data = [  # [obj_uuids, rel_obj_uuids, values, details, state]
                    ...
                    None,
                    None,
                    'Restricción repetido {} veces'.format(record['duplicate_total']),
                    error_state]
