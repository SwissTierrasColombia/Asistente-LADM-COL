...
from asistente_ladm_col.config.quality_rule_config import (QR_FDCR4002,
                                                           QRE_FDCR4002E01)
...
class QRFDCParcelConditionInvalid(AbstractLogicQualityRule):
    """
    This quality rule validates that the condition of the parcel is not null
    """
    _ERROR_01 = QRE_FDCR4002E01  # Condition of the parcel is null

    def __init__(self):
        AbstractLogicQualityRule.__init__(self)

        self._id = QR_FDCR4002
        self._name = "La condición de predio no puede ser null"
        self._tags = ["fdc", "captura", "campo", "lógica", "negocio", "predio", "condición"]
    ...
    def _validate(self, db, db_qr, layer_dict, tolerance, **kwargs):
        ...
        res, records = ladm_queries.get_invalid_null_values(db, db.names.FDC_PARCEL_T, db.names.FDC_PARCEL_T_CONDITION_F)
        count = len(records)
        ...
        if res:
            ...
            for record in records:
                error_data = [  # [obj_uuids, rel_obj_uuids, values, details, state]
                    ...
                    None,
                    None,
                    'La condición de predio no puede ser null',
                    error_state]
        ...
        if count > 0:
            ...
            msg = QCoreApplication.translate("QualityRules", "{} parcels with invalid parcel condition were found.").format(count)
        else:
            res_type = EnumQualityRuleResult.SUCCESS
            msg = QCoreApplication.translate("QualityRules", "All parcels have valid parcel condition.")
