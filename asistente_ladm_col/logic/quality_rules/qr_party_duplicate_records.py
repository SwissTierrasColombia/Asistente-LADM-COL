...
class QRDuplicatePartyRecords(AbstractLogicQualityRule):
    ...
    def __init__(self):
        ...
        self._name = "Revisar que la tabla interesado no tenga registros repetidos"
    ...
    def _validate(self, db, db_qr, layer_dict, tolerance, **kwargs):
        ...
        fields = LayerConfig.get_logic_consistency_tables(db.names).get(db.names.LC_PARTY_T)
        # Check party with duplicate records
        res, records = ladm_queries.get_duplicate_records_in_table(db,db.names.LC_PARTY_T,fields)
        count = len(records)
