                    ...
                    SELECT * FROM (
                        SELECT {id}, {parcel_type}, sum(count_terreno) sum_t, sum(count_construccion) sum_c, sum(count_unidadconstruccion) sum_uc FROM (
                            ...
                            FROM {schema}.{input_table} p left join {schema}.{join_table} ue on p.{id} = ue.{join_field}
                               ...
                               ({parcel_type}= (select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} = '{PARCEL_TYPE_NO_HORIZONTAL_PROPERTY}') AND (sum_t !=1 OR sum_uc != 0)) OR 
                               ({parcel_type} in (select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} in ('{PARCEL_TYPE_HORIZONTAL_PROPERTY_PARENT}', '{PARCEL_TYPE_CONDOMINIUM_PARENT}', '{PARCEL_TYPE_CEMETERY_PARENT}', '{PARCEL_TYPE_PUBLIC_USE}', '{PARCEL_TYPE_CONDOMINIUM_PARCEL_UNIT}')) AND (sum_t!=1 OR sum_uc > 0)) OR 
                               ({parcel_type} in (select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} in ('{PARCEL_TYPE_ROAD}', '{PARCEL_TYPE_CEMETERY_PARCEL_UNIT}')) AND (sum_t !=1 OR sum_uc > 0 OR sum_c > 0)) OR 
                               ({parcel_type}= (select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} = '{PARCEL_TYPE_HORIZONTAL_PROPERTY_PARCEL_UNIT}') AND (sum_t !=0 OR sum_c != 0 OR sum_uc = 0 )) OR 
                               ({parcel_type} in (select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} in ('{PARCEL_TYPE_HORIZONTAL_PROPERTY_MEJORA}', '{PARCEL_TYPE_NO_HORIZONTAL_PROPERTY_MEJORA}')) AND (sum_t !=0 OR sum_c != 1 OR sum_uc != 0))
                """.format(schema=schema, input_table=names.OP_PARCEL_T, join_table=names.COL_UE_BAUNIT_T,
                           OP_PARCEL_TYPE_D=names.OP_PARCEL_TYPE_D, ILICODE_F=names.ILICODE_F, PARCEL_TYPE_NO_HORIZONTAL_PROPERTY=names.PARCEL_TYPE_NO_HORIZONTAL_PROPERTY,
            ...
            'PARCEL_TYPE_AND_22_POSITION_OF_PARCEL_NUMBER_VALIDATION': {
                'query': """
                        ...
                        WHERE (p.{parcel_number} IS NOT NULL AND 
                               (substring(p.{parcel_number},22,1) != '0' AND p.{parcel_type}=(select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} = '{PARCEL_TYPE_NO_HORIZONTAL_PROPERTY}')) OR
                               (substring(p.{parcel_number},22,1) != '9' AND p.{parcel_type} in (select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} in ('{PARCEL_TYPE_HORIZONTAL_PROPERTY_PARENT}', '{PARCEL_TYPE_HORIZONTAL_PROPERTY_PARCEL_UNIT}'))) OR
                               (substring(p.{parcel_number},22,1) != '8' AND p.{parcel_type} in (select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} in ('{PARCEL_TYPE_CONDOMINIUM_PARENT}', '{PARCEL_TYPE_CONDOMINIUM_PARCEL_UNIT}'))) OR
                               (substring(p.{parcel_number},22,1) != '7' AND p.{parcel_type} in (select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} in ('{PARCEL_TYPE_CEMETERY_PARENT}', '{PARCEL_TYPE_CEMETERY_PARCEL_UNIT}'))) OR
                               (substring(p.{parcel_number},22,1) != '5' AND p.{parcel_type} in (select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} in ('{PARCEL_TYPE_HORIZONTAL_PROPERTY_MEJORA}', '{PARCEL_TYPE_NO_HORIZONTAL_PROPERTY_MEJORA}'))) OR
                               (substring(p.{parcel_number},22,1) != '4' AND p.{parcel_type}=(select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} = '{PARCEL_TYPE_ROAD}')) OR
                               (substring(p.{parcel_number},22,1) != '3' AND p.{parcel_type}=(select {id} from {schema}.{OP_PARCEL_TYPE_D} where {ILICODE_F} = '{PARCEL_TYPE_PUBLIC_USE}'))
