...
class PGLADMQuery(QGISLADMQuery):
    ...
    def get_group_party_fractions_that_do_not_make_one(db):
        query = """
                    SELECT {members_t_group_party_f} as agrupacion, string_agg({t_id}::text, ',') as miembros, round(SUM(parte)::numeric,15) suma_fracciones  FROM (
                    ...
                    HAVING round(SUM(parte)::numeric,15) != 1
                """.format(t_id=db.names.T_ID_F,
                           schema=db.schema,
    ...
    def get_inconsistent_building_units(db):
        query = """select uc.{t_ili_tid},
                    ...
                    case when split_part(ut.{ilicode}, '.', 1) != uct.{ilicode} then 1 else 0 end as dominio_general_uso,
                ...
                join {schema}.{lc_building_type_d} ct on uc.{lc_building_unit_t_building_type_f} = ct.{t_id}
                join {schema}.{lc_building_unit_type_d} uct on uc.{lc_building_unit_t_building_unit_type_f} = uct.{t_id}
                join {schema}.{lc_building_unit_use_d} ut on uc.{lc_building_unit_t_use_f} = ut.{t_id}
                where
                    ...
                    or split_part(ut.{ilicode}, '.', 1) != uct.{ilicode}
    ...
    def get_inconsistent_building_units_parcel(db):
        query = """ select uc.{t_ili_tid} as ucons_t_ili_tid,
                    ...
                    from {schema}.{lc_parcel_t} lp
                    join {schema}.{lc_condition_parcel_type_d} cp ON lp.{lc_parcel_t_condition_f} = cp.{t_id}
                    join {schema}.{lc_ue_baunit_t} ub on lp.{t_id} = ub.{lc_ue_baunit_t_parcel_f} and ub.{lc_ue_baunit_t_building_unit_f} is not null
                    join {schema}.{lc_building_unit_t} uc on uc.{t_id} = ub.{lc_ue_baunit_t_building_unit_f}
                    join {schema}.{lc_building_unit_use_d} ut on uc.{lc_building_unit_t_use_f} = ut.{t_id}
                    where split_part(ut.{ilicode}, '.', 1) != '{lc_building_unit_type_d_ilicode_f_annex_v}'
