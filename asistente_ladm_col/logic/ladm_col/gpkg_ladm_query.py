...
class GPKGLADMQuery(QGISLADMQuery):
    ...
    def get_uebaunit_parcel(db):
        query = """
            ...
            from (select p.{t_id}, p.{t_ili_tid}, {lc_parcel_t_parcel_type_f}, count({col_ue_baunit_t_lc_plot_f}) sum_t,
                    ...
                    from {lc_parcel_t} p
                    left join {col_ue_baunit_t} ue on p.{t_id} = ue.{col_ue_baunit_t_parcel_f}
                    group by p.{t_id}) as report
            join {lc_condition_parcel_type_d} cp on report.{lc_parcel_t_parcel_type_f} = cp.{t_id}
            where
    ...
    def get_parcels_with_invalid_parcel_type_and_22_position_number(db):
        query = """
            with parcels
            as (
                select p.{t_id}, p.{t_ili_tid}, {ilicode}, substr({lc_parcel_t_parcel_number_f},22,1) as pos22
                from {lc_parcel_t} p
                join {lc_condition_parcel_type_d} cpt on {lc_parcel_t_parcel_type_f} = cpt.{t_id}
            )
    ...
    def get_parcels_with_repeated_domain_right(db):
        query = """SELECT {t_ili_tid}, conteo.dominios
                   FROM {lc_parcel_t}, (SELECT {col_baunit_rrr_t_unit_f},
                                               count({lc_right_t_type_f}) as conteo,
                                               group_concat({t_ili_tid}, ';') as dominios
                                                            FROM {lc_right_t}
