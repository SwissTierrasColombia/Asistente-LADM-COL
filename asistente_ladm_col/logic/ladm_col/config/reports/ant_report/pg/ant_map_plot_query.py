...
def get_municipality_boundary(names, schema, plot_id, overview):
    ...
    query = """
            ...
            limite_municipio AS (SELECT geometria AS geom , nombre_municipio FROM {schema}.cc_limitemunicipio
                                 WHERE geometria && (SELECT ST_Expand(ST_Envelope(geometria), {scale_zoom}) FROM {schema}.{LC_PLOT_T} WHERE {T_ID_F} = {plot_id}))
            SELECT array_to_json(array_agg(features)) AS features
            FROM (
                SELECT f AS features
                FROM (
...
def get_urban_limit(names, schema, plot_id, overview):
    ...
    query = """
            WITH
            perimetro_urbano AS (
                SELECT geometria AS geom, nombre_geografico FROM {schema}.cc_perimetrourbano
                WHERE geometria && (SELECT ST_Expand(ST_Envelope(geometria), {scale_zoom}) FROM {schema}.{LC_PLOT_T} WHERE {T_ID_F} = {plot_id})
            )
            SELECT array_to_json(array_agg(features)) AS features
            FROM (
...
def get_road_nomenclature(names, schema, plot_id, overview):
    ...
    query = """
            ...
            nomenclatura_vial AS (
                SELECT (SELECT dispname FROM {schema}.cc_nomenclaturavial_tipo_via WHERE {T_ID_F} = tipo_via) || ' ' || numero_via AS nombre, geometria AS geom FROM {schema}.cc_nomenclaturavial
                WHERE geometria && (SELECT ST_Expand(ST_Envelope(geometria), {scale_zoom}) FROM {schema}.{LC_PLOT_T} WHERE {T_ID_F} = {plot_id})
            )
            SELECT array_to_json(array_agg(features)) AS features
            FROM (
