...
class QualityUtils(QObject):
    ...
    def check_boundary_points_covered_by_boundary_nodes(self, db, rule_name, translated_strings):
        ...
        if not layers:
            ...
            error_layer = QgsVectorLayer("Point?crs={}".format(get_crs_authid(layers[db.names.OP_BOUNDARY_POINT_T].sourceCrs())),
            ...
            features = self.get_boundary_points_features_not_covered_by_boundary_nodes(db, layers[db.names.OP_BOUNDARY_POINT_T], layers[db.names.OP_BOUNDARY_T], layers[db.names.POINT_BFS_T], error_layer, translated_strings, db.names.T_ID_F)
    ...
    def get_boundary_points_features_not_covered_by_boundary_nodes(self, db, boundary_point_layer, boundary_layer, point_bfs_layer, error_layer, translated_strings, id_field):
        ...
        boundary_nodes_layer = QgsVectorLayer("Point?crs={}".format(get_crs_authid(boundary_layer.sourceCrs())), 'unique boundary nodes', "memory")
    ...
    def check_boundary_nodes_covered_by_boundary_points(self, db, rule_name, translated_strings):
        ...
        if not layers:
            ...
            error_layer = QgsVectorLayer("Point?crs={}".format(get_crs_authid(layers[db.names.OP_BOUNDARY_T].sourceCrs())),
            ...
            features = self.get_boundary_nodes_features_not_covered_by_boundary_points(db, layers[db.names.OP_BOUNDARY_POINT_T], layers[db.names.OP_BOUNDARY_T], layers[db.names.POINT_BFS_T], error_layer, translated_strings, db.names.T_ID_F)
    ...
    def get_boundary_nodes_features_not_covered_by_boundary_points(self, db, boundary_point_layer, boundary_layer, point_bfs_layer, error_layer, translated_strings, id_field):
        ...
        boundary_nodes_unique_layer = QgsVectorLayer("Point?crs={}".format(get_crs_authid(boundary_layer.sourceCrs())), 'unique boundary nodes', "memory")
    ...
    def check_plot_nodes_covered_by_boundary_points(self, db, rule_name, translated_strings):
        ...
        if layers[db.names.OP_PLOT_T].featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("Point?crs={}".format(get_crs_authid(layers[db.names.OP_PLOT_T].sourceCrs())),
            ...
            features = self.get_boundary_points_features_not_covered_by_plot_nodes_and_viceversa(db, layers[db.names.OP_BOUNDARY_POINT_T], layers[db.names.OP_PLOT_T], error_layer, topology_rule, db.names.T_ID_F)
            error_layer.dataProvider().addFeatures(features)
    ...
    def get_boundary_points_features_not_covered_by_plot_nodes_and_viceversa(self, db, boundary_point_layer, plot_layer, error_layer, topology_rule, id_field):
        ...
        plot_nodes_layer = QgsVectorLayer("Point?crs={}".format(get_crs_authid(plot_layer.sourceCrs())), 'unique boundary nodes', "memory")
    ...
    def check_overlapping_points(self, db, point_layer_name, rule_name, translated_strings):
        ...
        """
        features = []
        point_layer = self.app.core.get_layer(db, point_layer_name, load=True)
        if not point_layer:
            return
        ...
        if point_layer.featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("Point?crs={}".format(get_crs_authid(point_layer.sourceCrs())),
    ...
    def check_plots_covered_by_boundaries(self, db, rule_name, translated_strings):
        ...
        if layers[db.names.OP_PLOT_T].featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("MultiLineString?crs={}".format(get_crs_authid(layers[db.names.OP_PLOT_T].sourceCrs())),
    ...
    def check_boundaries_covered_by_plots(self, db, rule_name, translated_strings):
        ...
        if layers[db.names.OP_BOUNDARY_T].featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("MultiLineString?crs={}".format(get_crs_authid(layers[db.names.OP_BOUNDARY_T].sourceCrs())),
    ...
    def check_overlapping_polygons(self, db, polygon_layer_name, rule_name, translated_strings):
        ...
        if polygon_layer:
            ...
            error_layer = QgsVectorLayer("Polygon?crs={}".format(get_crs_authid(polygon_layer.sourceCrs())),
                                         error_layer_name, "memory")
            data_provider = error_layer.dataProvider()
    ...
    def check_boundaries_are_not_split(self, db, rule_name, translated_strings):
        ...
        if boundary_layer.featureCount() == 0:
            ...
            if wrong_boundaries is None:
                ...
                error_layer = QgsVectorLayer("LineString?crs={}".format(get_crs_authid(boundary_layer.sourceCrs())),
    ...
    def check_missing_boundary_points_in_boundaries(self, db, translated_strings):
        ...
        points_selected = self.geometry.join_boundary_points_with_boundary_discard_nonmatching(layers[db.names.OP_BOUNDARY_POINT_T], layers[db.names.OP_BOUNDARY_T], db.names.T_ID_F)
    ...
    def check_missing_survey_points_in_buildings(self, db):
        ...
        error_layer = QgsVectorLayer("Point?crs={}".format(get_crs_authid(layers[db.names.OP_BUILDING_T].sourceCrs())),
    ...
    def check_dangles_in_boundaries(self, db, rule_name, translated_strings):
        ...
        if boundary_layer.featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("Point?crs={}".format(get_crs_authid(boundary_layer.sourceCrs())),
    ...
    def check_right_of_way_overlaps_buildings(self, db, rule_name, translated_strings):
        ...
        if layers[db.names.OP_RIGHT_OF_WAY_T].featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("MultiPolygon?crs={}".format(get_crs_authid(layers[db.names.OP_BUILDING_T].sourceCrs())),
    ...
    def check_gaps_in_plots(self, db, rule_name, translated_strings):
        ...
        if plot_layer.featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("MultiPolygon?crs={}".format(get_crs_authid(plot_layer.sourceCrs())),
    ...
    def check_multiparts_in_right_of_way(self, db, rule_name, translated_strings):
        ...
        if right_of_way_layer.featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("Polygon?crs={}".format(get_crs_authid(right_of_way_layer.sourceCrs())),
    ...
    def get_dangle_ids(self, boundary_layer):
        ...
        overlapping_points = self.geometry.get_overlapping_points(end_points)
    ...
    def check_building_within_plots(self, db, rule_name, translated_strings):
        ...
        if layers[db.names.OP_BUILDING_T].featureCount() == 0:
            ...
            buildings_with_no_plot, buildings_not_within_plot = self.geometry.get_buildings_out_of_plots(layers[db.names.OP_BUILDING_T], layers[db.names.OP_PLOT_T], db.names.T_ID_F)
    ...
    def check_building_unit_within_plots(self, db, rule_name, translated_strings):
        ...
        if layers[db.names.OP_BUILDING_UNIT_T].featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("MultiPolygon?crs={}".format(get_crs_authid(layers[db.names.OP_BUILDING_UNIT_T].sourceCrs())),
            ...
            building_units_with_no_plot, building_units_not_within_plot = self.geometry.get_buildings_out_of_plots(layers[db.names.OP_BUILDING_UNIT_T], layers[db.names.OP_PLOT_T], db.names.T_ID_F)
