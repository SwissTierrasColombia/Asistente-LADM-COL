...
class LineQualityRules:
            ...
            if overlapping is None:
                ...
                if type(error_point_layer) is QgsVectorLayer:
                    error_point_layer.setName("{} (point intersections)".format(
                        self.translated_strings[QualityRuleEnum.Line.OVERLAPS_IN_BOUNDARIES]
                    ))
        ...
        if boundary_layer.featureCount() == 0:
            ...
            if wrong_boundaries is None:
                ...
                error_layer = QgsVectorLayer("LineString?crs={}".format(boundary_layer.sourceCrs().authid()),
                                self.translated_strings[QualityRuleEnum.Line.BOUNDARIES_ARE_NOT_SPLIT],
                                "memory")
    ...
    def check_boundaries_covered_by_plots(self, db):
        ...
        if layers[db.names.OP_BOUNDARY_T][LAYER].featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("MultiLineString?crs={}".format(layers[db.names.OP_BOUNDARY_T][LAYER].sourceCrs().authid()),
                                         self.translated_strings[QualityRuleEnum.Line.BOUNDARIES_COVERED_BY_PLOTS],
                                         "memory")
    ...
    def check_boundary_nodes_covered_by_boundary_points(self, db):
        ...
        if not layers:
            ...
            features = self.get_boundary_nodes_features_not_covered_by_boundary_points(db, layers[db.names.OP_BOUNDARY_POINT_T][LAYER], layers[db.names.OP_BOUNDARY_T][LAYER], layers[db.names.POINT_BFS_T][LAYER], error_layer, db.names.T_ID_F)
    ...
    def check_dangles_in_boundaries(self, db):
        ...
        if boundary_layer.featureCount() == 0:
            ...
            error_layer = QgsVectorLayer("Point?crs={}".format(boundary_layer.sourceCrs().authid()),
                                self.translated_strings[QualityRuleEnum.Line.DANGLES_IN_BOUNDARIES],
                                "memory")
            pr = error_layer.dataProvider()
            pr.addAttributes([QgsField("boundary_id", QVariant.Int)])
            error_layer.updateFields()
