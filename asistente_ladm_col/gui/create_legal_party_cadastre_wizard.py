...
class CreateLegalPartyCadastreWizard(QWizard, WIZARD_UI):
    ...
    def adjust_page_1_controls(self):
        if self.rad_refactor.isChecked():
            ...
            finish_button_text = QCoreApplication.translate("CreateLegalPartyCadastreWizard", "Import")
            self.txt_help_page_1.setHtml(self.help_strings.get_refactor_help_string(LEGAL_PARTY_TABLE, False))

        elif self.rad_create_manually.isChecked():
            self.lbl_refactor_source.setEnabled(False)
            self.mMapLayerComboBox.setEnabled(False)
            finish_button_text = QCoreApplication.translate("CreateLegalPartyCadastreWizard", "Create")
            self.txt_help_page_1.setHtml(self.help_strings.WIZ_CREATE_LEGAL_PARTY_CADASTRE_PAGE_1_OPTION_FORM)
