...
WIZARD_UI = get_ui_class('wizards/property_record_card/wiz_create_property_record_card_prc.ui')


class CreatePropertyRecordCardPRCWizard(QWizard, WIZARD_UI):
    WIZARD_NAME = "CreatePropertyRecordCardWizard"
    WIZARD_TOOL_NAME = QCoreApplication.translate(WIZARD_NAME, "Create property record card")
    ...
    def adjust_page_1_controls(self):
        ...
        self.cbo_mapping.addItems(self.qgis_utils.get_field_mappings_file_names(PROPERTY_RECORD_CARD_TABLE))
        ...
        if self.rad_refactor.isChecked():
            if self.mMapLayerComboBox.currentLayer() is not None:
                ...
                if res_etl_model:
                    ...
                    self.qgis_utils.save_field_mapping(PROPERTY_RECORD_CARD_TABLE)
    ...
    def disconnect_signals(self):
        # QGIS APP
        try:
            self._layers[PROPERTY_RECORD_CARD_TABLE][LAYER].committedFeaturesAdded.disconnect(self.finish_feature_creation)
        except:
            pass

    def edit_feature(self):
        self.iface.layerTreeView().setCurrentLayer(self._layers[PROPERTY_RECORD_CARD_TABLE][LAYER])
        self._layers[PROPERTY_RECORD_CARD_TABLE][LAYER].committedFeaturesAdded.connect(self.finish_feature_creation)
        self.open_form(self._layers[PROPERTY_RECORD_CARD_TABLE][LAYER])
    ...
    def finish_feature_creation(self, layerId, features):
        ...
        if not self._layers[PROPERTY_RECORD_CARD_TABLE][LAYER].getFeature(fid).isValid():
            message = QCoreApplication.translate(self.WIZARD_NAME,
                                                 "'{}' tool has been closed. Feature not found in layer {}... It's not posible create a boundary. ").format(self.WIZARD_TOOL_NAME, PROPERTY_RECORD_CARD_TABLE)
            self.log.logMessage("Feature not found in layer {} ...".format(PROPERTY_RECORD_CARD_TABLE), PLUGIN_NAME, Qgis.Warning)
        else:
            feature_tid = self._layers[PROPERTY_RECORD_CARD_TABLE][LAYER].getFeature(fid)[ID_FIELD]
            message = QCoreApplication.translate(self.WIZARD_NAME,
                                                 "The new property record card (t_id={}) was successfully created ").format(feature_tid)
