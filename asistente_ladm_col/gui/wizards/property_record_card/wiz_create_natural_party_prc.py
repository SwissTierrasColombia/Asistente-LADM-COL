...
WIZARD_UI = get_ui_class('wizards/property_record_card/wiz_create_natural_party_prc.ui')


class CreateNaturalPartyPRCWizard(QWizard, WIZARD_UI):
    WIZARD_NAME = "CreateNaturalPartyWizard"
    WIZARD_TOOL_NAME = QCoreApplication.translate(WIZARD_NAME, "Create natural party")
    ...
    def disconnect_signals(self):
        # QGIS APP
        try:
            self._layers[NATURAL_PARTY_TABLE][LAYER].committedFeaturesAdded.disconnect(self.finish_feature_creation)
        except:
            pass

    def edit_feature(self):
        self.iface.layerTreeView().setCurrentLayer(self._layers[NATURAL_PARTY_TABLE][LAYER])
        self._layers[NATURAL_PARTY_TABLE][LAYER].committedFeaturesAdded.connect(self.finish_feature_creation)
        self.open_form(self._layers[NATURAL_PARTY_TABLE][LAYER])
    ...
    def finish_feature_creation(self, layerId, features):
        ...
        if not self._layers[NATURAL_PARTY_TABLE][LAYER].getFeature(fid).isValid():
            message = QCoreApplication.translate(self.WIZARD_NAME,
                                                 "'{}' tool has been closed. Feature not found in layer {}... It's not posible create a boundary. ").format(self.WIZARD_TOOL_NAME, NATURAL_PARTY_TABLE)
            self.log.logMessage("Feature not found in layer {} ...".format(NATURAL_PARTY_TABLE), PLUGIN_NAME, Qgis.Warning)
        else:
            feature_tid = self._layers[NATURAL_PARTY_TABLE][LAYER].getFeature(fid)[ID_FIELD]
            message = QCoreApplication.translate(self.WIZARD_NAME,
                                                 "The new natural party card (t_id={}) was successfully created ").format(feature_tid)

        self._layers[NATURAL_PARTY_TABLE][LAYER].committedFeaturesAdded.disconnect(self.finish_feature_creation)
        self.log.logMessage("Natural party's committedFeaturesAdded SIGNAL disconnected", PLUGIN_NAME, Qgis.Info)
