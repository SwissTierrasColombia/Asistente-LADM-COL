...
class CreateNuclearFamilyPRCWizard(QWizard, WIZARD_UI):
    WIZARD_NAME = "CreateNuclearFamilyWizard"
    WIZARD_TOOL_NAME = QCoreApplication.translate(WIZARD_NAME, "Create nuclear family")
    ...
    def disconnect_signals(self):
        # QGIS APP
        try:
            self._layers[NUCLEAR_FAMILY_TABLE][LAYER].committedFeaturesAdded.disconnect(self.finish_feature_creation)
        except:
            pass

    def edit_feature(self):
        self.iface.layerTreeView().setCurrentLayer(self._layers[NUCLEAR_FAMILY_TABLE][LAYER])
        self._layers[NUCLEAR_FAMILY_TABLE][LAYER].committedFeaturesAdded.connect(self.finish_feature_creation)
        self.open_form(self._layers[NUCLEAR_FAMILY_TABLE][LAYER])
    ...
    def finish_feature_creation(self, layerId, features):
        ...
        if not self._layers[NUCLEAR_FAMILY_TABLE][LAYER].getFeature(fid).isValid():
            message = QCoreApplication.translate(self.WIZARD_NAME,
                                                 "'{}' tool has been closed. Feature not found in layer {}... It's not posible create a boundary. ").format(self.WIZARD_TOOL_NAME, NUCLEAR_FAMILY_TABLE)
            self.log.logMessage("Feature not found in layer {} ...".format(NUCLEAR_FAMILY_TABLE), PLUGIN_NAME, Qgis.Warning)
        else:
            feature_tid = self._layers[NUCLEAR_FAMILY_TABLE][LAYER].getFeature(fid)[ID_FIELD]
            message = QCoreApplication.translate(self.WIZARD_NAME,
                                                 "The new nuclear family (t_id={}) was successfully created ").format(feature_tid)

        self._layers[NUCLEAR_FAMILY_TABLE][LAYER].committedFeaturesAdded.disconnect(self.finish_feature_creation)
        self.log.logMessage("Nuclear Family's committedFeaturesAdded SIGNAL disconnected", PLUGIN_NAME, Qgis.Info)
