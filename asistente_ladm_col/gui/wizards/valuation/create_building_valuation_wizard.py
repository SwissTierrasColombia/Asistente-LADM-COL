...
class CreateBuildingValuationWizard(QWizard, WIZARD_UI):
    ...
    def disconnect_signals(self):
        # QGIS APP
        try:
            self._layers[VALUATION_BUILDING_TABLE][LAYER].committedFeaturesAdded.disconnect(self.finish_feature_creation)
        except:
            pass

    def edit_feature(self):
        self.iface.layerTreeView().setCurrentLayer(self._layers[VALUATION_BUILDING_TABLE][LAYER])
        self._layers[VALUATION_BUILDING_TABLE][LAYER].committedFeaturesAdded.connect(self.finish_feature_creation)
        self.open_form(self._layers[VALUATION_BUILDING_TABLE][LAYER])
    ...
    def finish_feature_creation(self, layerId, features):
        ...
        if not self._layers[VALUATION_BUILDING_TABLE][LAYER].getFeature(fid).isValid():
            message = QCoreApplication.translate(self.WIZARD_NAME,
                                                 "'{}' tool has been closed. Feature not found in layer {}... It's not posible create a boundary. ").format(self.WIZARD_TOOL_NAME, VALUATION_BUILDING_TABLE)
            self.log.logMessage("Feature not found in layer {} ...".format(VALUATION_BUILDING_TABLE), PLUGIN_NAME, Qgis.Warning)
        else:
            feature_tid = self._layers[VALUATION_BUILDING_TABLE][LAYER].getFeature(fid)[ID_FIELD]
            message = QCoreApplication.translate(self.WIZARD_NAME,
