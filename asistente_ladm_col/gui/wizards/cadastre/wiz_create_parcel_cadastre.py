...
class CreateParcelCadastreWizard(MultiPageWizardFactory,
    ...
    def post_save(self, features):
        ...
        if len(features) != 1:
            ...
            if not self._layers[self.EDITING_LAYER_NAME][LAYER].getFeature(fid).isValid():
                ...
                if self.names.OP_PLOT_T in self.names.get_constraint_types_of_parcels()[self.dict_parcel_type[self.cb_parcel_type.currentText()]]:
                    if self.names.get_constraint_types_of_parcels()[self.dict_parcel_type[self.cb_parcel_type.currentText()]][self.names.OP_PLOT_T] is not None:
                        plot_ids = [f[self.names.T_ID_F] for f in self._layers[self.names.OP_PLOT_T][LAYER].selectedFeatures()]
                else:
                    plot_ids = [f[self.names.T_ID_F] for f in self._layers[self.names.OP_PLOT_T][LAYER].selectedFeatures()]

                if self.names.OP_BUILDING_T in self.names.get_constraint_types_of_parcels()[self.dict_parcel_type[self.cb_parcel_type.currentText()]]:
                    if self.names.get_constraint_types_of_parcels()[self.dict_parcel_type[self.cb_parcel_type.currentText()]][self.names.OP_BUILDING_T] is not None:
                        building_ids = [f[self.names.T_ID_F] for f in self._layers[self.names.OP_BUILDING_T][LAYER].selectedFeatures()]
                else:
                    building_ids = [f[self.names.T_ID_F] for f in self._layers[self.names.OP_BUILDING_T][LAYER].selectedFeatures()]

                if self.names.OP_BUILDING_UNIT_T in self.names.get_constraint_types_of_parcels()[self.dict_parcel_type[self.cb_parcel_type.currentText()]]:
                    if self.names.get_constraint_types_of_parcels()[self.dict_parcel_type[self.cb_parcel_type.currentText()]][self.names.OP_BUILDING_UNIT_T] is not None:
    ...
    def check_selected_features(self):
        ...
        for spatial_unit in self.names.get_constraint_types_of_parcels()[parcel_type]:
            _layer = self._spatial_unit_layers[spatial_unit]

            _color = CSS_COLOR_OKAY_LABEL

            if self.names.get_constraint_types_of_parcels()[parcel_type][spatial_unit] == 1 and not _layer.selectedFeatureCount() == 1:
                _color = CSS_COLOR_ERROR_LABEL
            elif self.names.get_constraint_types_of_parcels()[parcel_type][spatial_unit] == '+' and _layer.selectedFeatureCount() < 1:
                _color = CSS_COLOR_ERROR_LABEL
            elif self.names.get_constraint_types_of_parcels()[parcel_type][spatial_unit] is None:
    ...
    def adjust_page_2_controls(self):
        ...
        if self.cb_parcel_type.count() == 0:
            for feature in self._layers[self.names.OP_PARCEL_TYPE_T][LAYER].getFeatures():
                if feature[self.names.ILICODE_F] in self.names.get_constraint_types_of_parcels():
    ...
    def exec_form(self, layer):
        ...
        feature[self.names.OP_PARCEL_T_PARCEL_TYPE_F] = self.cb_parcel_type.currentText()
        ...
        if dialog.exec_():
            ...
            parcel_condition_field_idx = layer.getFeature(fid).fieldNameIndex(self.names.OP_PARCEL_T_PARCEL_TYPE_F)
    ...
    def validate_type_of_parcel(self, parcel_type):
        ...
        for spatial_unit in self.names.get_constraint_types_of_parcels()[parcel_type]:
            if self.names.get_constraint_types_of_parcels()[parcel_type][spatial_unit] is None:
                if spatial_unit == self.names.OP_PLOT_T:
