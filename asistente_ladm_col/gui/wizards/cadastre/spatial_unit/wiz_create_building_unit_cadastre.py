...
WIZARD_UI = get_ui_class('wizards/cadastre/spatial_unit/wiz_create_building_unit_cadastre.ui')
...
class CreateBuildingUnitCadastreWizard(QWizard, WIZARD_UI):
    ...
    def exec_form(self, layer, f_id):
        ...
        message = None
        if dialog.exec_():
            message = QCoreApplication.translate(self.WIZARD_NAME, "Building unit was created, but no changes have been saved.")
            self.rollback_changes = False
        else:
