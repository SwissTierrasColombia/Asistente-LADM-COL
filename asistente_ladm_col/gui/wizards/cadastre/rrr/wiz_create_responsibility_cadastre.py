...
WIZARD_UI = get_ui_class('wizards/cadastre/rrr/wiz_create_responsibility_cadastre.ui')
...
class CreateResponsibilityCadastreWizard(QWizard, WIZARD_UI):
    ...
    def finish_feature_creation(self, layerId, features):
        ...
        if len(features) != 1:
            message = QCoreApplication.translate(self.WIZARD_NAME,
                                                 "'{}' tool has been closed. We should have got only one responsibility... We cannot do anything with {} responsibility").format(self.WIZARD_TOOL_NAME, len(features))
    ...
    def exec_form(self, layer):
        ...
        if dialog.exec_():
            ...
            if not saved:
                ...
                self.qgis_utils.message_emitted.emit(
                    QCoreApplication.translate(self.WIZARD_NAME,
                                               "Error while saving changes. Responsibility could not be created."),
                    Qgis.Warning)
