...
class TestGetLayers(unittest.TestCase):
    ...
    def test_get_layer(self):
        ...
        RELATED_TABLES = {self.db_connection.names.OP_BOUNDARY_POINT_T: [self.db_connection.names.OP_AGREEMENT_TYPE_D,
                                                           self.db_connection.names.OP_PHOTO_IDENTIFICATION_TYPE_D,
                                                           self.db_connection.names.COL_PRODUCTION_METHOD_TYPE_D,
                                                           self.db_connection.names.COL_INTERPOLATION_TYPE_D,
                                                           self.db_connection.names.OP_LOCATION_POINT_TYPE_D,
                                                           self.db_connection.names.OP_POINT_TYPE_D,
                                                           self.db_connection.names.COL_MONUMENTATION_TYPE_D,
                                                           self.db_connection.names.OP_BOUNDARY_POINT_T],
                          self.db_connection.names.OP_PLOT_T: [self.db_connection.names.COL_SURFACE_RELATION_TYPE_D,
                                                 self.db_connection.names.COL_DIMENSION_TYPE_D,
                                                 self.db_connection.names.OP_PLOT_T]
                          }
        ...
        for layer in [self.db_connection.names.OP_BOUNDARY_POINT_T, self.db_connection.names.OP_PLOT_T]:
        ...
        print("First for {} layer".format(self.db_connection.names.OP_BOUNDARY_POINT_T))

        for pre_load in [self.db_connection.names.OP_AGREEMENT_TYPE_D, self.db_connection.names.COL_MONUMENTATION_TYPE_D]: # preload some layers
            self.qgis_utils.get_layer(self.db_connection, pre_load, load=True)

        self.qgis_utils.get_layer(self.db_connection, self.db_connection.names.OP_BOUNDARY_POINT_T, load=True)

        # check number if element in Layer Tree and needed element are the same.
        loaded_layers_tree_names = len(RELATED_TABLES[self.db_connection.names.OP_BOUNDARY_POINT_T])
        ...
        for pre_load in [self.db_connection.names.OP_AGREEMENT_TYPE_D, self.db_connection.names.COL_MONUMENTATION_TYPE_D]:
            self.qgis_utils.get_layer(self.db_connection, pre_load, load=True)
        layer_tree_elements = len([layer.name() for layer in self.qgis_utils.get_ladm_layers_from_layer_tree(self.db_connection)])
        self.assertEqual(loaded_layers_tree_names, layer_tree_elements, "Duplicate layers found... This is an error!!!")
        QgsProject.instance().clear()

        print("Then for {} layer".format(self.db_connection.names.OP_PLOT_T))
        for pre_load in [self.db_connection.names.COL_SURFACE_RELATION_TYPE_D]: # preload some layers
            self.qgis_utils.get_layer(self.db_connection, pre_load, load=True)

        self.qgis_utils.get_layer(self.db_connection, self.db_connection.names.OP_PLOT_T, geometry_type=QgsWkbTypes.PolygonGeometry, load=True)

        # check number if element in Layer Tree and needed element are the same.
        loaded_layers_tree_names = len(RELATED_TABLES[self.db_connection.names.OP_PLOT_T])
        layer_tree_elements = len([layer.name() for layer in self.qgis_utils.get_ladm_layers_from_layer_tree(self.db_connection)])
        ...
        for pre_load in [self.db_connection.names.COL_SURFACE_RELATION_TYPE_D]:
            self.qgis_utils.get_layer(self.db_connection, pre_load, load=True)
        layer_tree_elements = len([layer.name() for layer in self.qgis_utils.get_ladm_layers_from_layer_tree(self.db_connection)])
        self.assertEqual(loaded_layers_tree_names, layer_tree_elements, "Duplicate layers found... This is an error!!!")
        QgsProject.instance().clear()
        ...
        self.qgis_utils.get_layer(self.db_connection, self.db_connection.names.OP_PLOT_T, load=True)
