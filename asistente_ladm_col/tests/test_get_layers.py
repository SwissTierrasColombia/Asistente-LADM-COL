...
class TestGetLayers(unittest.TestCase):
    ...
    def test_get_layer(self):
        ...
        for layer in [self.db_pg.names.OP_BOUNDARY_POINT_T, self.db_pg.names.OP_PLOT_T]:
            ...
            loaded_layers_tree_names = [self.db_pg.get_ladm_layer_name(layer) for layer in self.qgis_utils.get_ladm_layers_by_register_type(self.db_pg, LayerRegisterType.IN_REGISTER)]
        ...
        layer_tree_elements = len([layer.name() for layer in self.qgis_utils.get_ladm_layers_by_register_type(self.db_pg, LayerRegisterType.IN_REGISTER)])
        ...
        self.qgis_utils.get_layer(self.db_pg, self.db_pg.names.OP_PLOT_T, load=True)
        toc_layers = [l for l in self.qgis_utils.get_ladm_layers_by_register_type(self.db_pg, LayerRegisterType.IN_REGISTER)]
