...
class TestChangeDetectionsSupplies(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.db_connection = get_dbconn('test_change_detections')
    ...
    def test_get_plots_related_to_parcels_supplies(self):
        ...
        for parcel_ids_test in parcel_ids_tests:
            plot_ids = self.ladm_data.get_plots_related_to_parcels_supplies(self.db_connection, parcel_ids_test, self.names.T_ID_F)
        ...
        count = 0
        for parcel_ids_test in parcel_ids_tests:
            plot_custom_field_ids = self.ladm_data.get_plots_related_to_parcels_supplies(self.db_connection, parcel_ids_test, self.names.GC_PLOT_T_DIGITAL_PLOT_AREA_F)
        ...
        layers = {self.names.GC_PLOT_T: {'name': self.names.GC_PLOT_T, 'geometry': QgsWkbTypes.PolygonGeometry, LAYER: None}}
    ...
    def test_get_parcels_related_to_plots_supplies(self):
        ...
        for plot_ids_test in plot_ids_tests:
            parcel_ids = self.ladm_data.get_parcels_related_to_plots_supplies(self.db_connection, plot_ids_test, self.names.T_ID_F)
            # We use assertCountEqual to compare if two lists are the same regardless of the order of their elements.
            # https://docs.python.org/3.2/library/unittest.html#unittest.TestCase.assertCountEqual
    ...
    def test_get_parcel_data_to_compare_changes_supplies(self):
        ...
        search_criterion = {self.names.GC_PARCEL_T_PARCEL_NUMBER_F: '253940000000000230099335131315'}
        ...
        search_criterion = {self.names.GC_PARCEL_T_FMI_F: '760ab38'}
        features = self.ladm_data.get_parcel_data_to_compare_changes_supplies(self.db_connection, search_criterion=search_criterion)
        self.normalize_responce(features)
        self.assertEqual(features, features_test)
